# Interactivity won't work without an ipynb, but placed here for reference
# in case there are any old grumpy men using this repo
...
def dashboard(skirmish_dice, assault_dice, raid_dice, xaxis, yaxis):
    ...
    pivot = df.pivot_table(index=yaxis, columns=xaxis, values='prob',
    ...
    cax = ax_hm.imshow(z, aspect='auto', origin='upper', cmap='viridis',
                       extent=[x[0] - 0.5, x[-1] + 0.5, y[-1] + 0.5,
                               y[0] - 0.5])
    for i in range(len(y)):
        for j in range(len(x)):
            ax_hm.text(x[j], y[i], f"{z[i, j]:.2f}", ha='center',
                       va='center', color='w', fontsize=8)
    ax_hm.set_xticks(x)
    ax_hm.set_yticks(y)
    ax_hm.set_xlabel(xaxis.capitalize())
    ax_hm.set_ylabel(yaxis.capitalize())
    ax_hm.set_title(f"Probability Heatmap: {xaxis.capitalize()} vs "
                    f"{yaxis.capitalize()}")
    ax_hm.invert_yaxis()
    fig.colorbar(cax, ax=ax_hm, label='Probability', fraction=0.035,
                 pad=0.03)
    ...
    fig.suptitle(
        f"Dice: Skirmish={skirmish_dice}, Assault={assault_dice}, "
        f"Raid={raid_dice}",
        fontsize=15
    )
...
interact(
    dashboard,
    skirmish_dice=IntSlider(min=0, max=6, step=1, value=2,
                            description='Skirmish'),
    assault_dice=IntSlider(min=0, max=6, step=1, value=2,
                           description='Assault'),
