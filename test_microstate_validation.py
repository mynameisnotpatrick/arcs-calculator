...
This test generates ALL possible microstates for dice combinations up to 10 total dice
and validates that our optimized probability calculations match the brute force results.
"""
...
def brute_force_all_microstates(num_skirmish, num_assault, num_raid, fresh_targets=0, convert_intercepts=False):
	"""
	Brute force calculation of all possible dice roll outcomes (microstates).
	
	This generates every single possible combination of dice rolls using itertools.product
	...
	probabilities = [count / total_microstates for count in macrostate_counts.values()]
...
def compare_probability_results(optimized_result, brute_force_result, tolerance=1e-10):
	"""
	...
	assert len(opt_macrostates) == len(bf_macrostates), \
		f"Different number of macrostates: optimized={len(opt_macrostates)}, brute_force={len(bf_macrostates)}"
	...
	assert not missing_in_opt, f"Macrostates missing in optimized: {missing_in_opt}"
	assert not missing_in_bf, f"Macrostates missing in brute force: {missing_in_bf}"
	...
	for state in opt_states:
		...
		assert error < tolerance, \
			f"Probability mismatch for state '{state}': optimized={opt_prob:.12f}, brute_force={bf_prob:.12f}, error={error:.2e}"
...
class TestMicrostateValidation:
	...
	def test_single_die_types(self):
		...
		for num_skirmish, num_assault, num_raid in test_cases:
			print(f"\nTesting {num_skirmish} skirmish, {num_assault} assault, {num_raid} raid dice...")
			
			# Get results from both methods
			optimized = arcs_funcs.compute_probabilities(num_skirmish, num_assault, num_raid)
			brute_force = brute_force_all_microstates(num_skirmish, num_assault, num_raid)
	...
	def test_convert_intercepts(self):
		...
		for num_skirmish, num_assault, num_raid, fresh_targets, convert_intercepts in test_cases:
			print(f"\nTesting {num_skirmish} skirmish, {num_assault} assault, {num_raid} raid dice with {fresh_targets} fresh targets...")
			
			optimized = arcs_funcs.compute_probabilities(
				num_skirmish, num_assault, num_raid, fresh_targets, convert_intercepts
			)
	...
	def test_large_combinations(self):
		...
		for total_dice in [9, 10]:
			for skirmish in range(0, min(6, total_dice) + 1):  # Max 6 skirmish dice
				for assault in range(0, min(6, total_dice - skirmish) + 1):  # Max 6 assault dice
		...
		for num_skirmish, num_assault, num_raid in test_cases:
			...
			expected_microstates = (2 ** num_skirmish) * (6 ** (num_assault + num_raid))
			
			print(f"\nTesting {num_skirmish} skirmish, {num_assault} assault, {num_raid} raid dice...")
			print(f"  Total dice: {total_dice}, Expected microstates: {expected_microstates:,}")
	...
	def test_probability_sum_equals_one(self):
		"""Verify that all probabilities sum to 1.0 for various combinations."""
		test_cases = [
			(2, 2, 2),
			(3, 2, 1), 
			(1, 3, 2),
		...
		for num_skirmish, num_assault, num_raid in test_cases:
			_, probs, *_ = arcs_funcs.compute_probabilities(num_skirmish, num_assault, num_raid)
			total_prob = sum(probs)
			
			assert abs(total_prob - 1.0) < 1e-10, \
				f"Probabilities don't sum to 1.0: got {total_prob:.12f} for {num_skirmish},{num_assault},{num_raid} dice"
			
			print(f"Probabilities sum to 1.0 for {num_skirmish},{num_assault},{num_raid} dice (sum={total_prob:.12f})")
